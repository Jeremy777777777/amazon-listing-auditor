...
HEADERS = [
    "严重程度", "问题类型", "产品名称", "内部编号", "Seller", "ASIN",
...
def _write_excel(results: list[AuditResult], path: Path) -> None:
    ...
    sheet.auto_filter.ref = f"A8:R{max(sheet.max_row, 8)}"
    if sheet.max_row > 8:
        table = Table(displayName="AuditIssues", ref=f"A8:R{sheet.max_row}")
        table.tableStyleInfo = TableStyleInfo(name="TableStyleMedium2", showRowStripes=True, showFirstColumn=False, showLastColumn=False)
        sheet.add_table(table)
    widths = [11, 27, 42, 15, 11, 14, 24, 34, 32, 34, 44, 34, 38, 44, 18, 38, 18, 16]
    for index, width in enumerate(widths, 1):
        sheet.column_dimensions[get_column_letter(index)].width = width
    yellow = PatternFill("solid", fgColor="FFF2CC")
    for row in sheet.iter_rows(min_row=9, min_col=15, max_col=18):
        for cell in row:
            cell.fill = yellow
    validation = DataValidation(type="list", formula1='"待审核,确认错误,已修改,False Positive"', allow_blank=False)
    sheet.add_data_validation(validation)
    validation.add(f"O9:O{max(sheet.max_row, 9)}")
    for formula, color in [("$A9=\"CRITICAL\"", "E06666"), ("$A9=\"HIGH\"", "FCE5CD"), ("$A9=\"REVIEW\"", "FFF2CC")]:
        sheet.conditional_formatting.add(f"A9:R{max(sheet.max_row, 9)}", FormulaRule(formula=[formula], fill=PatternFill("solid", fgColor=color)))
